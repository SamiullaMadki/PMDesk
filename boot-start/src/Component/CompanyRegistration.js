...
export default function Registration() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
 const companyId = Math.random().toString().slice(2, 12); // generate unique id
    setFormData({ ...formData, companyId }); // add id to form data
    await fetch('http://localhost:3001/registercompany', {
      method: 'POST',
      headers: {
   ...
   navigate('/ProjectWorkspace', { replace: true });
  }
  return (
